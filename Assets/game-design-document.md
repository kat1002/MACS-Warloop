...
After an enemy has been killed, it has a 7% chance of spawning a powerup.
...
This example features only a single enemy type, as described below.
...
Enemy ships are destroyed when their health reaches zero; see details below.

## Enemy 1

### Spawn & Movements

Enemies appear randomly off-screen and move directly into the game area. Enemies must not overlap.

### Attack: Fire Bullet

Bullets fly straight, disappearing once they've gone the full screen. Enemies only fire bullets when they're fully visible on the screen. Bullet speed = 2 * enemy movement speed.
One bullet is fired every 5 seconds when an enemy appears.
...
- UI must display current player scores.
